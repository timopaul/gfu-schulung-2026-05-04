...
@section('title', isset($event) ? __('Edit Event ":event"', ['event' => $event]) : __('Create New Event'))

@section('content')
    <div class="max-w-2xl mx-auto py-8">
        <div class="bg-white shadow-md rounded-lg p-6">
            <h2 class="text-xl font-semibold mb-6 text-gray-800">{{ isset($event) ? __('Edit Event ":event"', ['event' => $event]) : __('Create New Event') }}</h2>

            <form action="{{ isset($event) ? route('events.save', ['event' => $event]) : route('events.store') }}"
                ...
                <div>
                    ...
                    <select name="trainer_id"
                        ...
                        @foreach($trainers as $trainer)
                            <option
                                value="{{ $trainer->getKey() }}" {{ old('trainer_id', isset($event) ? $event->trainer?->getKey() : null) == $trainer->getKey() ? 'selected' : '' }}>
                                {{ $trainer->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="location"
                           value="{{ old('location', isset($event) ? $event->location : null) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2">
                    @error('location')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="start_date"
                               value="{{ old('start_date', isset($event) ? $event->start_date->format('Y-m-d') : null) }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2">
                        @error('start_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date"
                               value="{{ old('end_date', isset($event) ? $event->end_date->format('Y-m-d') : null) }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm border p-2">
                        @error('end_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="flex items-center w-full pt-4">
                    @if(isset($event))
                        <a href="{{ route('events.remove', ['event' => $event]) }}"
                           class="gap-x-2 text-sm mr-auto inline-flex items-center rounded-lg bg-white text-gray-700 hover:text-gray-900 transition-all px-6 py-2 shadow-sm ring-1 ring-inset ring-gray-300 hover:ring-gray-500 hover:bg-gray-50">Event
                            löschen</a>
                    @endif
                    <a href="{{ url()->previous() }}"
                       class="gap-x-2 text-sm inline-flex items-center rounded-lg bg-white text-gray-700 hover:text-gray-900 transition-all px-6 py-2 shadow-sm ring-1 ring-inset ring-gray-300 hover:ring-gray-500 hover:bg-gray-50 ml-auto mr-4">zurück</a>
                    <button type="submit"
                            class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg transition shadow-md">
